...
import money.com.gettingmoney.bai.activity.MoniAllActivity;
import money.com.gettingmoney.bai.activity.NewsDetailActivity;
...
import money.com.gettingmoney.bean.Index_detalsBean;
...
//activity_indexdetiails_act
//指数详情的界面
public class Indexdetails_act extends Activity implements View.OnClickListener {
    ...
    private int xiala;
    private int type;
    String stockId, shock_names;//从上一页传递过来的stockId（股票id）和股票的名称//从指数界面点进去的
    ...
    //指数分时数据
    private List<Indexdetials_Klinebean.ListBean> stockHistoryListBeans = new ArrayList<>();
    XAxis xAxisK;
    YAxis axisLeftK;
    YAxis axisRightK;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_indexdetails_act);

        Toast.makeText(Indexdetails_act.this, "这是从指数的fragment中进入到板块的详情界面中的", Toast.LENGTH_SHORT).show();
        Event();

        stockId = getIntent().getStringExtra("stockId");
        shock_names = getIntent().getStringExtra("shock_name");
        dialog = new LoadingDialog(Indexdetails_act.this, "正在加载");
        dialog.show();
        //k线图
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.buy:
                Intent intent1 = new Intent(Indexdetails_act.this, MoniAllActivity.class);
                intent1.putExtra("type", 0);
                startActivity(intent1);
                break;
            case R.id.sell:

                Intent intent2 = new Intent(Indexdetails_act.this, MoniAllActivity.class);
                intent2.putExtra("type", 1);
                startActivity(intent2);
                break;
            case R.id.kill_order:
                Intent intent4 = new Intent(Indexdetails_act.this, MoniAllActivity.class);
            ...
            case R.id.share:
                Toast.makeText(Indexdetails_act.this, "你选择了分享", Toast.LENGTH_SHORT).show();

                break;
            case R.id.option:
                if (Login()) {
                    dialog = new LoadingDialog(Indexdetails_act.this, "正在添加股票中");
                    dialog.show();
                    createStock();
                }
                break;
            case R.id.detials_kline:
                ...
                Intent intent = new Intent(Indexdetails_act.this, money.com.gettingmoney.Test_Kline.KLineActivity.class);
                intent.putExtra("stockId", stockId);
                intent.putExtra("type", "指数");//在指数的详情页面传递type过去 在下一个页面接受这个字段并且判断是否是从指数列表里点进来的
    ...
    void newsList() {
        Runnable newsList = new Runnable() {
            ...
            public void run() {
                ...
                util.newslist(dialog, type, page, num, ShareUtil.getInstance().getUserNumber(Indexdetails_act.this), new MyXutils.XCallBack() {
                    @Override
    ...
    private void initEvent() {
        ...
        mAdapter = new CommonAdapter<homeNewsitem>(Indexdetails_act.this, null, R.layout.bai_homenews_list_items) {
            @Override
            public void convert(ViewHolder helper, final homeNewsitem item) {
                ...
                helper.getConvertView().setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        startActivity(new Intent(Indexdetails_act.this, NewsDetailActivity.class).putExtra(NewsDetailActivity.KEY_ID, item.getId()));
    ...
    void createStock() {
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                indexuti.createStock(dialog, ShareUtil.getInstance().getUserNumber(Indexdetails_act.this), userStocks, new MyXutils.XCallBack() {
                    @Override
                    public void onResponse(String result) {

                        dialog.close();
                        Toast.makeText(Indexdetails_act.this, "添加股票成功", Toast.LENGTH_SHORT).show();
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                indexuti.indexdetail(stockId, new MyXutils.XCallBack() {
                    ...
                    public void onResponse(String result) {
                        //解析
                        Index_detalsBean Bean = new Gson().fromJson(result, new TypeToken<Index_detalsBean>() {
                        }.getType());

                        if (Bean.getHandicap().getTotalHands().length() > 0) {
                            now.setText(Bean.getHandicap().getTotalHands());
                        }
                        if (Bean.getHandicap().getRose().length() > 0 && Bean.getHandicap().getDowns().length() > 0) {
                            differRange_differ.setText(Bean.getHandicap().getRose() + "  " + Bean.getHandicap().getDowns());
                        }

                        if (String.valueOf(Bean.getHandicap().getHighest()).length() > 0) {
                            hight.setText(String.valueOf(Bean.getHandicap().getHighest()));
                        }
                        if (String.valueOf(Bean.getHandicap().getMinimum()).length() > 0) {
                            low.setText(String.valueOf(Bean.getHandicap().getMinimum()));
                        }
                        if (String.valueOf(Bean.getHandicap().getThisOpen()).length() > 0) {

                            open.setText(String.valueOf(Bean.getHandicap().getThisOpen()));
                        }
                        if (String.valueOf(Bean.getHandicap().getZuoshou()).length() > 0) {
                            preClose.setText(String.valueOf(Bean.getHandicap().getZuoshou()));
                        }

                        if (Bean.getHandicap().getDownsLimit() != null) {
                            sell1Money.setText(Bean.getHandicap().getDownsLimit());
                        }
                        if (Bean.getHandicap().getRoseLimit() != null) {
                            buy1Money.setText(Bean.getHandicap().getRoseLimit());
                        }
                        userStocks.setSharesCode(Bean.getCode());
                        //绑定值
                    }
                });
            }
    ...
    //判断是否有登录
    boolean Login() {
        if (ShareUtil.getInstance().getUserNumber(Indexdetails_act.this).length() < 1 || ("").equals(ShareUtil.getInstance().getUserNumber(Indexdetails_act.this))) {
            Toast.makeText(Indexdetails_act.this, "你还没有登录", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
    ...
    private void setData(List<Indexdetials_Klinebean.ListBean> mData) {
